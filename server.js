...
/* ✅ CORS (required for CodePen / Netlify / Vercel / browser requests) */
app.use(cors({
  origin: "*",
...
================================ */
if (!MONGO_URI) {
  console.log("❌ MONGO_URI missing in Render environment variables");
}
...
const AttendanceSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ["Present", "Absent", "On Duty (O/D)", "Leave"],
    required: true
  }
}, { timestamps: true });
...
app.post("/signup", async (req, res) => {
  try {
    ...
    const existingRange = await Settings.findOne({ key: "attendanceRange" });
    if (!existingRange) {
...
/* ✅ Owner list users (no email returned) */
app.get("/owner/users", auth, onlyOwner, async (req, res) => {
  const users = await User.find().sort({ createdAt: -1 });
  res.json({ success: true, users: users.map(u => safeUser(u)) });
});
